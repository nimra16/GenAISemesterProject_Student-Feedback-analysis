...
class PDF(FPDF):
    ...
    def add_aspect_info(self, aspect, discussed_count, total_respondents, wordcloud_image, comments, aspect_terms):
        ...
        for comment in comments:
            for term in aspect_terms:
                if pd.notna(term):
                    comment = comment.replace(term, f"<b>{term}</b>")
            # Bordered Comment Box
            self.set_fill_color(240, 240, 240)
            self.multi_cell(0, 10, comment, border=1, align='L', fill=True)
            self.ln(2)
...
if uploaded_file:
    ...
    if selected_teacher:
        ...
        for aspect, path in wordcloud_paths:
            ...
            pdf.add_aspect_info(aspect, discussed_count, total_respondents, path, aspect_df['Comments'].tolist(), aspect_df[f"{aspect}_terms"].tolist())
